    ...
    let recipe = StepDocument(
        ...
        sections: [
            StepSection(title: "Cook", steps: [
                ...
                Step(title: "Simmer 20 min", kind: .timer, duration: .minutes(20), timer: StepTimer(duration: .minutes(20)))
            ])
        ]
    )
    ...
    let film = StepDocument(
        ...
        sections: [
            StepSection(title: "Develop", steps: [
                ...
                Step(title: "Develop 9 min", detail: "Agitate for 10 sec each minute.", kind: .timer, duration: .minutes(9), timer: StepTimer(duration: .minutes(9)))
            ])
        ]
    )
